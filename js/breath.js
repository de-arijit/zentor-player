...
// Audio calls happen directly in button handlers (user gesture).
// ============================================================
'use strict';
...
const BreathGate = (() => {
  ...
  // Called directly from onclick — this IS the user gesture
  function start() {
    ...
    document.getElementById('bProtoRow').style.cssText += ';opacity:.4;pointer-events:none';
    document.getElementById('bCycleWrap').style.cssText += ';opacity:.4;pointer-events:none';

    running = true; paused = false; round = 0; phaseIdx = 0;

    // Play intro DIRECTLY here (user gesture context preserved)
    // After intro ends (or 8s max) → start phases
    Voice.intro(proto, () => _runPhase(), 8000);
  }
  ...
  function _runPhase() {
    ...
    // Round cue at start of each round, phase cue for subsequent phases
    if (phaseIdx === 0) {
      Voice.round(proto, round);
      setTimeout(() => { if (!paused) Voice.phase(ph.voiceKey); }, 1600);
    } else {
      Voice.phase(ph.voiceKey);
    }
  ...
  function _finish() {
    ...
    Voice.done(proto, () => {
      APP_showScreen('sScreen');
      // Auto-start session after a brief pause
      setTimeout(() => Session.start(), 700);
    }, 7000);
  }
...
const BreathStandalone = (() => {
  ...
  function selectProto(key) {
    ...
    const btn = document.getElementById('bwpb-' + key);
    if (btn) btn.classList.add('active');
    const p = PROTOS[key];
  ...
  function _runPhase() {
    ...
    document.getElementById('bwOrb').className = 'orb ' + p.color + ' ' + ph.anim;
    ...
    let cnt = ph.dur; _setText('bwCount', cnt);
    clearInterval(iv);
    iv = setInterval(() => {
      if (paused) return;
