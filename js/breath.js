...
const BreathGate = (() => {
  ...
  function start() {
    ...
    // Intro audio then run — 6s max fallback
    let started = false;
    const go = () => { if (!started) { started = true; _runPhase(); } };
    Voice.intro(proto, go, 6000);
    setTimeout(go, 6000);
  }
  ...
  function _runPhase() {
    ...
    // CRITICAL: force reflow to restart CSS animation
